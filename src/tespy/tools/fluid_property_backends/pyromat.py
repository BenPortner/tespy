...
TESPy uses the Pyromat python interface for all fluid property functions.
...
default_h = mph2o.h
default_Th = mph2o.T_h
h_corr = - default_h(T=298.15, p=1e5) - 285830 * mph2o.mw()
def mph2O_corrected_h(*args, **kwargs):
    return default_h(*args, **kwargs) + h_corr
mph2o.h = mph2O_corrected_h

def mph2o_corrected_Th(h, p=None, d=None, quality=False, debug=False):
    h = h - h_corr
    return default_Th(h=h, p=p, d=d, quality=quality, debug=debug)
mph2o.T_h = mph2o_corrected_Th
...
class Memorise:
    ...
    def add_fluids(fluids, memorise_fluid_properties=True):
        ...
        ----------
        fluids : dict
            Dict of fluid and corresponding Pyromat back end for fluid
        ...
        if memorise_fluid_properties and num_fl > 0:
            ...
            msg = "Added fluids " + ", ".join(fl) + " to memorise lookup tables."
            logging.debug(msg)
        ...
        for f in fluids.keys():
            ...
            # handle Air
            if f in ["Air"]:
                Memorise.state[f] = pm.get("ig.air")
                gas_constants[f] = 8.314459861448581  # J/(mol*K)

            # handle water
            elif f in ["H2O", "water"]:
                Memorise.water = f
                Memorise.state[f] = mph2o
                gas_constants[f] = Memorise.state[f].R() * Memorise.state[f].mw()  # J/(mol*K)

            # handle other fluids
            else:
                try:
                    Memorise.state[f] = pm.get("ig." + f)
                    gas_constants[f] = Memorise.state[f].R() * Memorise.state[f].mw()  # J/(mol*K)
                except ValueError:
                    msg = (
            ...
            msg = "Created Pyromat object for fluid " + f + "."
            logging.debug(msg)

            Memorise.back_end[f] = "pyromat"

            # value range for fluid properties
            p_range = Memorise.state[f].data.get("plim", [1e4, 1e8])
            T_range = Memorise.state[f].Tlim()
            Memorise.value_range[f] = [p_range[0], p_range[1], T_range[0], T_range[1]]

            molar_masses[f] = Memorise.state[f].mw() # kg/mol
    ...
    def del_memory(fluids):
        ...
        try:
            # delete memory
            Memorise.s_ph[fl] = Memorise.s_ph[fl][Memorise.s_ph[fl][:, -1] > threshold]
            Memorise.s_ph[fl][:, -1] = 0

            Memorise.T_ph[fl] = Memorise.T_ph[fl][Memorise.T_ph[fl][:, -1] > threshold]
            Memorise.T_ph[fl][:, -1] = 0

            Memorise.T_ps[fl] = Memorise.T_ps[fl][Memorise.T_ps[fl][:, -1] > threshold]
            Memorise.T_ps[fl][:, -1] = 0

            Memorise.v_ph[fl] = Memorise.v_ph[fl][Memorise.v_ph[fl][:, -1] > threshold]
            Memorise.v_ph[fl][:, -1] = 0

            msg = (
                "Dropping not frequently used fluid property values from "
...
def T_mix_ph(flow, T0=675):
    ...
    Uses Pyromat interface for pure fluids, newton algorithm for mixtures:
    ...
    if fluid is None:
        ...
        valmin = max([Memorise.value_range[f][2] for f in fl if flow[3][f] > err]) + 0.1
...
def T_ph(p, h, fluid):
    ...
    T : float
        Temperature T / K.
    """
    if Memorise.state[fluid].data["class"] == "ig2":
        return Memorise.state[fluid].T_h(h=h)[0]
    else:
        return Memorise.state[fluid].T_h(h=h, p=p)[0]
...
def dT_mix_ph_dfluid(flow, T0=675):
    ...
    for fluid, x in flow[3].items():
        if x > err:
            up[3][fluid] += d
            lo[3][fluid] -= d
            vec_deriv += [(T_mix_ph(up, T0=T0) - T_mix_ph(lo, T0=T0)) / (2 * d)]
            up[3][fluid] -= d
            lo[3][fluid] += d
        else:
            vec_deriv += [0]
...
def T_mix_ps(flow, s, T0=675):
    ...
    if fluid is None:
        ...
        val = newton(
            s_mix_pT, ds_mix_pdT, flow, s, val0=T0, valmin=valmin, valmax=4000, imax=10
        )
    ...
    if memorisation:
        new = np.asarray([[flow[1], flow[2]] + list(flow[3].values()) + [s, val, 0]])
        # memorise the newly calculated value
...
def T_ps(p, s, fluid):
    ...
    T : float
       Temperature T / K.
    """
    return Memorise.state[fluid].T_s(s=s, p=p)[0]
...
def h_mix_pT(flow, T, force_gas=False):
    ...
    if fluid_name is None:
        ...
        x_i = {fluid: y / (molar_masses[fluid] * n) for fluid, y in flow[3].items()}

        water = Memorise.water
        if water is not None and not force_gas and flow[3][water] > err:
            y_i_gas, x_i_gas, y_water_liq, x_water_liq = cond_check(
                flow[3], x_i, flow[1], n, T
            )
        ...
        for fluid, y in y_i_gas.items():
            if y > err:
                ...
                if fluid == water and y_water_liq > 0:
                    h += Memorise.state[fluid].h(T=T, x=0) * y_water_liq
                    h += Memorise.state[fluid].h(T=T, x=1) * y * (1 - y_water_liq)

                # vapor
                else:
                    h += (
                        h_pT(flow[1] * x_i_gas[fluid], T, fluid, force_gas)
                        * y
                        * (1 - y_water_liq)
                    )
...
def h_pT(p, T, fluid, force_gas=False):
    ...
    h : float
        Specific enthalpy h / (J/kg).
    """
    T_max = Memorise.value_range[fluid][3]
    if T > T_max and fluid == Memorise.water:
        obj = igh2o
    else:
        obj = Memorise.state[fluid]

    if force_gas:
        if T < get_T_crit(fluid):
            h = obj.h(p=p, T=T)[0]
            h_sat = obj.h(x=1, T=T)[0]
            return max(h, h_sat)

    return obj.h(p=p, T=T)[0]
...
def h_ps(p, s, fluid):
    ...
    """
    try:
        T_s = Memorise.state[fluid].T_s(s=s, p=p)[0]
    except pm.utility.PMParamError:
        T_s = Memorise.state[fluid].Tlim(p=p)[1]
    return Memorise.state[fluid].h(T=T_s, p=p)[0]
...
def h_mix_pQ(flow, Q):
    ...
    try:
        return Memorise.state[fluid].h(p=flow[1], x=Q)[0]
    except ValueError:
        p_crit = get_p_crit(fluid)
        return Memorise.state[fluid].h(p=p_crit * 0.99, x=Q)[0]
...
def get_p_crit(fluid):
    """
    Get critical point pressure. Pyromat currently supports critical pressure for H2O only.
    All other components are treated as ideal gases.
    ...
    """
    if "pc" in Memorise.state[fluid].data:
        return Memorise.state[fluid].data["pc"]
    else:
        return 0
...
def get_T_crit(fluid):
    """
    Get critical point temperature. Pyromat currently supports critical temperature for H2O only.
    All other components are treated as ideal gases.
    ...
    """
    if "Tc" in Memorise.state[fluid].data:
        return Memorise.state[fluid].data["Tc"]
    else:
        return 0
...
def T_bp_p(flow):
    ...
    p_crit = get_p_crit(fluid)
    if flow[1] > p_crit:
        return Memorise.state[fluid].T(p=p_crit * 0.99, x=1)[0]
    else:
        return Memorise.state[fluid].T(p=flow[1], x=1)[0]
...
def cond_check(y_i, x_i, p, n, T):
    ...
    if T < get_T_crit(water_label):
        p_sat = Memorise.state[water_label].ps(T=T)

        pp_water = p * y_i[water_label] / (molar_masses[water_label] * n)
        ...
        if p_sat < pp_water:
            ...
            y_water_liq = (
                x_water_liq
                * molar_masses[water_label]
                / (sum([x * molar_masses[fluid] for fluid, x in x_i.items()]))
            )

            M = sum([x * molar_masses[fluid] for fluid, x in x_i_gas.items()])
            y_i_gas = {
                fluid: x / M * molar_masses[fluid] for fluid, x in x_i_gas.items()
            }
...
def d_ph(p, h, fluid):
    ...
    """
    if Memorise.state[fluid].data["class"] == "ig2":
        T_h = Memorise.state[fluid].T_h(h=h)[0]
    else:
        T_h = Memorise.state[fluid].T_h(h=h, p=p)[0]
    return Memorise.state[fluid].d(T=T_h, p=p)[0]
...
def Q_ph(p, h, fluid):
    ...
    try:
        if Memorise.state[fluid].data["class"] == "ig2":
            ...
            T, x = Memorise.state[fluid].T_h(h=h, p=p, quality=True)
            return x[0]
...
def d_pT(p, T, fluid):
    ...
    d : float
        Density d / (kg/:math:`\mathrm{m}^3`).
    """
    return Memorise.state[fluid].d(p=p, T=T)[0]
...
def s_ph(p, h, fluid):
    ...
    if Memorise.state[fluid].data["class"] == "ig2":
        ...
        T_h = Memorise.state[fluid].T_h(h=h, p=p)[0]
    return Memorise.state[fluid].s(T=T_h, p=p)[0]
...
def s_mix_pT(flow, T, force_gas=False):
    ...
    if fluid_name is None:
        ...
        for fluid, y in y_i_gas.items():
            if y > err:
                if fluid == water and y_water_liq > 0:
                    s += Memorise.state[water].s(x=1, T=T) * y * (1 - y_water_liq)
                    s += Memorise.state[water].s(x=0, T=T) * y_water_liq
...
def s_pT(p, T, fluid, force_gas):
    ...
    if force_gas:
        if T < get_T_crit(fluid):
            s = obj.s(p=p, T=T)[0]
            s_sat = obj.s(x=1, T=T)[0]
            return max(s, s_sat)

    return obj.s(p=p, T=T)[0]
